...
1. **先判斷使用者有沒有指名一個「結構」。** 結構指的是他自己說出口的「想怎麼看這件事」，不是他的心情。訊號例如：
   - 講到時間順序：來龍去脈、怎麼走到今天、接下來三個月會怎麼發展
   - 有兩個明確的選項擺在一起比較
   - 明說只想要一句話、一個方向
   - 想一次看整體狀態、看能量哪裡堵住
   - 想從幾個他指定的層面分別去看

   只要問題帶著這種訊號，就從清單裡挑「使用時機」最吻合那個結構的牌陣。**這一步完全不要考慮 ★，即使選到的不是專屬牌陣也照選。**

   **只是在描述心情——很累、很亂、走不出來、不知道要什麼、一直在逃避——不算指名結構。** 那些直接進第 2 步，不要在這一步硬塞一個聽起來也很情緒的通用牌陣給他。

   另外兩個常被誤判的：問題本身寫得短，不等於他「只想要一句話」；心情說不上來，也不等於他要求「看整體」。這兩件事都要他自己講出來才算。

2. 問題只描述情緒與處境時，**先只看標了 ★ 的那一個**——那是這位塔羅師最擅長的儀式，它的「使用時機」寫的就是這類情緒處境。只要沾得上邊就選它。

3. 只有在 ★ 的使用時機和他的處境完全對不上時，才回到清單其他項目挑最接近的。

判斷時看問題背後真正的處境，不是字面關鍵字。
