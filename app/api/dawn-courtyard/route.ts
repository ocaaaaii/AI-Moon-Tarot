/**
 * POST /api/dawn-courtyard
 * Sacred Realms region — 黎明庭園 (Akira)'s single-reply ritual endpoint.
...
import { DAWN_COURTYARD_SYSTEM_PROMPT } from "@/lib/omikuji/dawnCourtyardPrompt";
...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  try {
    const message = await client.messages.create({
      ...
      system: DAWN_COURTYARD_SYSTEM_PROMPT,
      messages: [
        { role: "user", content: `使用者寫下他覺得自己絕對做不到的事：「${worry}」` },
      ],
    });
